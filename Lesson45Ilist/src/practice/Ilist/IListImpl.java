...
public class IListImpl <E> implements IList <E>{
    ...
    // fields
    private Object[] elements;// массив для списка объекта
    ...
    public E remove (int index) {
        ...
        elements[size] = null; // После выполнения System.arraycopy, когда элементы сдвигаются на одну позицию назад,
        // последний элемент становится дублированным на предпоследней позиции. Затирание последнего элемента
        // elements[size] = null; выполняется для очистки этой дублированной позиции, иначе в списке может остаться
        // ссылка на ненужный объект, что может вызвать утечку памяти.

        // Этот шаг необходим для правильной работы списка и предотвращения утечек памяти. В Java объекты не удаляются
        // сразу после вызова remove, и сборка мусора может произойти позже. Поэтому затирание последнего элемента
        // является хорошей практикой для избежания утечек памяти.
        return el;
    }
    ...
    public E get(int index) {
        checkIndex(index);
        return (E) elements[index]; // (E) casting to E type
    }
    ...
    public int indexOf(Object o) {
        ...
        } else { // когда в листе объект null
            for (int i = 0; i < size; i++) {
                if (o == (elements[i])) {
                    return i;
                }
