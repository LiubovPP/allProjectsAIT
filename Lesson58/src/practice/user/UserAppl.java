...
//  Найти всех пользователей младше 30 лет с именем, начинающимся на букву "A".
        List<User> userList30 =findUserByPredicate (userList, user -> user.getAge() > 30 && user.getName().startsWith("A")); // .charAt(0) == 'A'
        // печать
    ...
    private static List<User> findUserByPredicate(List<User> userList, Predicate <User> predicate) {
        ...
        for (User user: userList) {
            if (predicate.test(user)) { // если тест пройден, то добавляем
                resList.add(user);
            }
        }
        return resList;
